...
struct config {
        ...
        /*
         * Optional bearer token for /adm section.
         * When set, clients may authenticate with:
         *   Authorization: Bearer <token>
         * instead of HTTP Basic auth. This is particularly useful for
         * EventSource (SSE) clients, which cannot set custom headers —
         * the Web UI fetches a short-lived token via /api/v1/auth/token
         * and uses it in the SSE URL query string ?token=...
         * If NULL, bearer auth is disabled.
         */
