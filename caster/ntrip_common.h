...
struct ntrip_state {
	...
	/* Captured from the Sec-WebSocket-Key HTTP header during request
	 * parsing. Used by /api/v1/logs/ws to compute the Sec-WebSocket-Accept
	 * response header for the WebSocket handshake. NULL if not present.
	 * Freed in ntrip_free(). */
