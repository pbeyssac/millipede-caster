 ...
 * "Bearer":  RFC 6750, OAuth 2.0 token. The token is stored in *user
 *            and *password is set to NULL. The caller is responsible
 *            for comparing the token against the expected value.
...
int http_decode_auth(char *value, int *scheme_basic, char **user, char **password) {
        ...
        *p++ = '\0';
        ...
        if (!strcasecmp(value, "Basic"))
                r_scheme_basic = 1;
        else if (!strcasecmp(value, "Bearer"))
                r_scheme_bearer = 1;
        ...
        if (r_scheme_bearer) {
                /* "Bearer <token>" — store the raw token in *user,
                 * leave *password as NULL so callers can distinguish
                 * Bearer from Basic by checking *password. */
                auth = p;
                while (*p && !isspace(*p)) p++;
                *p = '\0';
                auth = mystrdup(auth);
